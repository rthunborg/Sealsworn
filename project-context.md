...
- Registered autoloads (unchanged through Epic 4): `GameSession`, `SceneManager`, `SaveManager`, `AudioManager`, `SettingsManager`, `Diagnostics`. Keep them thin; they delegate gameplay decisions to domain services. Generation and run-progression added NO new autoload — `LevelGenerator`/`ManualSeedLoader`/`RouteGenerator`, every run-domain command, and `RunOrchestrator` are pure `RefCounted` services/DTOs called directly. `SaveManager` gained thin route-position delegators (`autosave_route_position`, `resume_route_position` -> `RunResumeService`) but owns no run logic.
- Static-content storage (through Epic 4): `godot/data/source/` and `godot/data/resources/` are STILL EMPTY. Every definition (enemies, weapons, supports, level recipes) is a code constant authored as a baseline `_baseline_definitions()` array on its repository, registered through the `ContentRepository` boundary — there is NO `.tres`/JSON content pipeline yet, and Epic 4 added none (route nodes/types are code constants on `RouteNode`/`RouteGenerator`, not content resources). The JSON-source -> typed-Resource mirror is a later (Epic 6) decision; do not introduce it early. The `data/` roots are still asserted as required structure by `test_project_structure.gd`.
...
Last Updated: 2026-06-23 (refreshed after Epic 4: run progression & route — the `scripts/run/` run-progression model + `RunOrchestrator`, `RouteGenerator` under `scripts/generation/route/` drawing the `map` stream for an 8-12-non-boss-node forward-only DAG + terminal boss (constant 8-tier depth, width-only variation, `INTERIOR_COLUMN_COUNT = 6` branch-point guarantee), the take-a-`RunState` run-command idiom + single-code/metadata-reason error model living under `scripts/core/commands/`, reveal-on-arrival soft-lock prevention + idempotent cleared-set lockstep, MVP node-type placeholder resolution + the `run_started`/`run_completed` event lifecycle, the board-free `RunSnapshot.from_route_position` save/resume reusing the 4.1 bridge under the 23-key gate with a symmetric seed cross-check, the inert-run-RNG v0 limitation, and the 3.7 lossless-decimal-string idiom ported into `DomainEvent._has_decimal_string_payload`)
