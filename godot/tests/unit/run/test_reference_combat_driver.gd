...
const APPROVED_MEDIUM_LIVE_COMBAT_SEED_CATALOG: Array[Dictionary] = [
	...
	{
		"seed": 24680,
		"notes": "Medium/elite board, 2 iron_cultist + 2 gate_brute (4 melee bodies). Winnable by all three classes (warrior ~39 / pyromancer ~15 / ranger ~20 rounds): the warrior commits to the melee bodies one at a time; the ranged classes kite the melee group down the flank routes. STORY 14.1 RE-PIN (justified, human-ratified 2026-07-17): the prior Medium seer+melee seed 512 became un-winnable BY THE REFERENCE KITE HEURISTIC after corpse-clearing — a deterministic, legitimate consequence (dead bodies now vacate their cells, so the melee pursuers path THROUGH corpse cells and the ranger kite policy no longer converges within MAX_ROUNDS; warrior + pyromancer still win 512). 24680 restores an all-three-class Medium NEUTRAL-melee winnability proof at 18 HP. Medium ash_seer coverage is restored by seed 1337 (below)."
	},
	{
		"seed": 1337,
		"notes": "Medium/elite board (14x12), 1 iron_cultist + 2 gate_brute + 1 ash_seer — a genuine SEER + MIXED-MELEE board (the seer+melee character the dropped 512 carried). Winnable by all three classes UNDER corpse-clear (warrior 28 / pyromancer 17 / ranger 27 rounds; all well under the 64-round cap): the ranger KITES the three melee bodies AND dodges the seer mark while the melee pursuers path through vacated corpse cells, the warrior commits to the melee bodies one at a time then mops up the stationary seer, the pyromancer lines up the firing angle. STORY 14.1 (code-review fix, human-directed 2026-07-17): RESTORES the Medium ash_seer winnability coverage the 512 -> 24680 re-pin had dropped — a live, in-repo PROOF that the ranger kite heuristic DOES converge on a Medium seer+melee board under corpse-clear (512's non-convergence was a proof-harness heuristic artifact on that specific layout, NOT a general enemy-mobility over-buff that degrades the kite playstyle). Deterministic (byte-identical across two runs; the search + a focused re-verify both confirm all three classes win with zero living enemies + the hero alive)."
	}
]
