...
func run() -> Dictionary:
	...
	_neutral_default_run_to_completion_is_byte_identical_to_a_second_run()
	return result()
...
# ---- AC1: the DEFAULT (non-live) run_to_completion is byte-identical -------------------------------

func _neutral_default_run_to_completion_is_byte_identical_to_a_second_run() -> void:
	# The 11.4 live affinity wiring is on the LIVE combat path only — the DEFAULT auto-resolve run_to_completion (the path
	# the seed-regression fingerprints depend on) must stay byte-identical. Drive two fresh runs to completion via the
	# DEFAULT path + assert the final `map`/`level` stream state matches (the interrupted==uninterrupted determinism the
	# live loop must not perturb).
	...
	assert_equal(first_result.succeeded, second_result.succeeded, "Two DEFAULT runs of the same seed reach the same terminal result.")
	assert_equal(first.streams.to_snapshot(), second.streams.to_snapshot(), "The DEFAULT run_to_completion stream state is byte-identical (the live affinity wiring does not perturb it).")
