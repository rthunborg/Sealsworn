...
# Story 8.5 Task 3 (AC1/AC2/AC3): the FirstDeathNarrativeBeat — the scene-free, skippable first-death narrative read DTO.
# The beat build is a PURE read (ZERO RNG, ZERO mutation); an exact pinned DICTIONARY_KEYS; a first-death input projects
# has_beat == true + the resolved line ("Good. You remembered how to die.") + is_skippable == true; a null / unresolvable /
# non-first-death input projects the fail-closed empty beat (has_beat == false); a simulated build+read+dismiss leaves the
# profile/run byte-identical (AC2 — a skip is a structural no-op); the beat is INDEPENDENT of RunSummary (building one never
# touches the other, AC3).
...
func run() -> Dictionary:
	_first_death_beat_projects_the_resolved_line()
...
func _first_death_beat_projects_the_resolved_line() -> void:
	# A first-death beat projects has_beat == true + the resolved display prose (FR61) + is_skippable == true (FR65).
	var beat: FirstDeathNarrativeBeat = FirstDeathNarrativeBeat.for_first_death()

	assert_true(beat.has_beat, "A first-death beat must have a meaningful beat.")
	assert_equal(String(beat.line_id), String(DomainEvent.FIRST_DEATH_LINE_ID), "The beat must carry the line-by-id.")
	assert_equal(beat.line, FirstDeathNarrativeBeat.FIRST_DEATH_LINE, "The beat must resolve the first-death display prose.")
	assert_equal(beat.line, "Good. You remembered how to die.", "The resolved line must be the FR61 first-death line.")
	assert_true(beat.is_skippable, "The first-death beat must be skippable (FR65).")


func _dictionary_key_set_is_exact_and_pinned() -> void:
	# The exact-key discipline (the RunSummary precedent): to_dictionary() projects EXACTLY the pinned DICTIONARY_KEYS set.
	var beat: FirstDeathNarrativeBeat = FirstDeathNarrativeBeat.for_first_death()
	var data: Dictionary = beat.to_dictionary()

	var actual_keys: Array = data.keys()
	actual_keys.sort()
	var expected_keys: Array = FirstDeathNarrativeBeat.DICTIONARY_KEYS.duplicate()
	expected_keys.sort()

	assert_equal(actual_keys, expected_keys, "FirstDeathNarrativeBeat.to_dictionary() must project EXACTLY the pinned DICTIONARY_KEYS set.")
	assert_equal(data.size(), FirstDeathNarrativeBeat.DICTIONARY_KEYS.size(), "FirstDeathNarrativeBeat.to_dictionary() must have no surprise keys.")

	# The empty beat also holds the pinned key shape (the exact-key contract holds for the fail-closed projection too).
	var empty_data: Dictionary = FirstDeathNarrativeBeat.new().to_dictionary()
...
func _empty_beat_is_fail_closed() -> void:
	...
	var beat: FirstDeathNarrativeBeat = FirstDeathNarrativeBeat.new()
...
func _unresolvable_line_id_projects_empty_beat() -> void:
	...
	# line).
	var unknown: FirstDeathNarrativeBeat = FirstDeathNarrativeBeat.for_first_death(&"not_a_real_line")
	...
	var blank: FirstDeathNarrativeBeat = FirstDeathNarrativeBeat.for_first_death(&"")
...
func _beat_from_event_projects_the_resolved_line() -> void:
	# The from_event convenience seam: a first_death_recorded event projects the populated beat; a wrong-type / null event
	# projects the fail-closed empty beat.
	var event: DomainEvent = DomainEvent.first_death_recorded(1, {
		"line_id": String(DomainEvent.FIRST_DEATH_LINE_ID),
		"is_skippable": true,
		"profile_id": "default"
	})
	var beat: FirstDeathNarrativeBeat = FirstDeathNarrativeBeat.from_event(event)
	assert_true(beat.has_beat, "from_event on a first_death_recorded event must project a populated beat.")
	assert_equal(beat.line, "Good. You remembered how to die.", "from_event must resolve the first-death line.")
	assert_true(beat.is_skippable, "from_event must carry the event's skippable flag.")

	# A null event -> empty beat.
	assert_false(FirstDeathNarrativeBeat.from_event(null).has_beat, "from_event on a null event must project the empty beat.")

	# A wrong-type event (run_failed) -> empty beat.
	var wrong: DomainEvent = DomainEvent.run_failed(1, {"cause": "hero_death"})
	assert_false(FirstDeathNarrativeBeat.from_event(wrong).has_beat, "from_event on a non-first-death event must project the empty beat.")


func _build_read_dismiss_is_a_structural_no_op() -> void:
	# AC2 "skipping does not change rewards, unlocks, or tactical state": building + reading + a simulated dismiss of the
	# beat leaves the profile + run byte-identical (the DTO owns no truth to change; there is NO skip command). The line
	# delivery (this DTO) is SEPARATE from the flag mutation (RecordFirstDeathCommand), so a skip cannot mutate the flag.
	var run: RunState = _failed_run(2, 4242, false)
	var profile: ProfileSnapshot = ProfileSnapshot.new()
	# Record the first death FIRST (the flag mutation is the command's job), THEN snapshot state before touching the beat.
	RecordFirstDeathCommand.new(profile, 1).execute(run)
	...
	assert_true(profile.first_death_recorded, "The first-death flag stays set whether or not the beat is shown/skipped.")
...
func _repeated_builds_are_byte_identical() -> void:
	...
	var first: Dictionary = FirstDeathNarrativeBeat.for_first_death().to_dictionary()
	var second: Dictionary = FirstDeathNarrativeBeat.for_first_death().to_dictionary()
	...
	first["line"] = "mutated"
	assert_equal(FirstDeathNarrativeBeat.for_first_death().line, "Good. You remembered how to die.", "Mutating a returned dict must not perturb a fresh build.")


func _beat_is_independent_of_run_summary() -> void:
	# AC3: the beat is a SEPARATE optional surface — building it never touches RunSummary and RunSummary carries NO narrative
	# field. Building both from the SAME failed run keeps them independent (the summary key set is unchanged; the summary has
	# no narrative key).
	var run: RunState = _failed_run(2, 4242, false)
	...
	# The summary carries NO narrative field (AC3 non-dependency fence).
	var summary_data: Dictionary = summary.to_dictionary()
	assert_false(summary_data.has("line"), "The RunSummary must NOT carry a narrative line field (AC3 — narrative is off the summary critical path).")
	...
	# The summary's top-level key set is UNCHANGED (still exactly RunSummary.DICTIONARY_KEYS — 8.5 added no narrative key).
	...
	assert_equal(summary_keys, expected_summary_keys, "8.5 must NOT add a narrative field to RunSummary (the 8.2/8.4 key pin stays green).")
