...
#   - PASS for a FAIR Darkness level: entrance safe at the reduced radius, no reachable hazard unseen at the reduced
#     radius (a fresh all-FLOOR generated Darkness board passes by construction — no hazard means nothing unseen hurts).
...
#   - FAIL LOUD for an UNFAIR Darkness level (AC3 "failures report seed, phase, and fairness reason"): a hand-built
#     Darkness candidate with a reachable HAZARD cell UNSEEN at the reduced radius fails with a stable fairness reason +
#     the seed + the phase. Spawning ON a hazard / ON an enemy fails too (the safe-first-reveal semantic).
...
func run() -> Dictionary:
	...
	_unseen_hazard_at_reduced_radius_fails_loud()
...
# ---- fail loud: unseen hazard at the reduced radius ----------------------------------------------

func _unseen_hazard_at_reduced_radius_fails_loud() -> void:
	# A Darkness board with a reachable HAZARD cell placed OUTSIDE the Darkness-reduced radius (so it is unseen from the
	# entrance at the reduced radius) — the hero would advance blind into it = unavoidable damage from unseen space. FAIL.
	# Reduced radius = 2; entrance at (1, corridor). Place a hazard far down the open corridor (well beyond radius 2) on
	# a reachable FLOOR cell.
	var width: int = 14
	var height: int = 12
	var grid: Array = _open_grid(width, height)
	var corridor_row: int = height / 2
	# Hazard at x=8 on the corridor row: distance from entrance (1,6) is 7 > reduced radius 2 -> unseen at the reduced
	# radius. It is reachable (open corridor). This is the FR58 violation Darkness's reduced radius introduces.
	...
	assert_true(check.is_error(), "AC3: a reachable hazard unseen at the reduced radius FAILS the fairness check.")
	assert_equal(String(check.metadata.get("fairness_reason")), String(DarknessFairnessQuery.REASON_UNSEEN_HAZARD), "AC3: the failure reports the unseen-hazard fairness reason.")
	assert_equal(String(check.metadata.get("seed")), "424242", "AC3: the failure reports the seed.")
	assert_false(String(check.metadata.get("phase", "")).is_empty(), "AC3: the failure reports a phase.")
	assert_true(check.metadata.has("hazard_cell"), "The failure carries the offending hazard cell (compact diagnostics).")
...
func _failure_carries_seed_phase_and_reason() -> void:
	...
	# AND the stable top-level error code, exercised via a decimal-string seed (the int64 discipline).
	var width: int = 14
	var height: int = 12
	var grid: Array = _open_grid(width, height)
	_set_terrain(grid,9, height / 2, BoardCell.Terrain.HAZARD)
	var board: BoardState = _board_from_grid(width, height, grid)
	var big_seed: String = "9223372036854775807"  # int64 max as a decimal string (the seed-string discipline).
	var check: ActionResult = DarknessFairnessQuery.new().check_board(board, &"darkness", _repository(), big_seed)
