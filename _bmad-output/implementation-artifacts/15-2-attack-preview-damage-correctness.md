...
**Human decisions (2026-08-06, Auto-GDS Phase 7 halt) — both `[Review][Decision]` items resolved as "accept as-is", no code change this story:**
- Decision 1 (command result metadata passthrough `expected_damage`): accepted as-is. Verified inert by the reviewer — no key-set pin, excluded from every event payload, no fingerprint move. Left in place.
- Decision 2 (armed-preview `warnings`/`explanation` report the weapon base, not the boosted total): accepted as-is for this story. The clarity nuance is handed to the OSG-1 on-device pass / the 15.11 copy pass, exactly as the reviewer proposed.
