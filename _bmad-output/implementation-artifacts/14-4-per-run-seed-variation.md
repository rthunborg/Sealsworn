...
- [x] **Task 4 — Seam unit test (AC1, AC2, AC3)**
  ...
  - [x] Use `str(...)` (never eager `String(nullable)`) in assert messages (14.1 retro test-honesty note — eager `String(nullable)` crashes on a null read and masks the real failure).

- [x] **Task 5 — Determinism / save gates held + suite green (AC2, AC3)**
  - [x] Confirm **no domain/RNG/save change**: `RunStartCommand`, `RunOrchestrator.start`, `RngStreamSet` (7 streams, `_derive_seed`), `RunSnapshot` (23 keys, `SCHEMA_VERSION == 1`), and every generation/route/finale file are **untouched**. No new `DomainEvent` (enum unchanged), no new named stream, no new draw site, no new autoload. The only production files touched are the new seam + the two presenters.
...
Claude Opus 4.8 (gds-dev-story); story context by Claude Opus 4.8 (gds-create-story)

### Debug Log References

- Headless import to register the new global class + generate its sidecar: `Godot_v4.6.3-stable_win64_console.exe --headless --path godot --import` → `RunSeedSource` registered; `godot/scripts/ui/flow/run_seed_source.gd.uid` written. (Pre-existing, unrelated `preset.0/1/2 export_files` config warnings ignored — they concern export presets, not this change.)
- Mandatory gate: `Godot_v4.6.3-stable_win64_console.exe --headless --path godot --scene res://tests/headless/test_runner.tscn --quit-after 10` → **201 PASS / 0 FAIL** (baseline 200 + the new seam test). False-PASS guard `SCRIPT ERROR|Parse Error|^FAIL` = clean; exactly the **6 documented stderr negatives** (int64-overflow ×2 in `test_manual_seed_loader`, malformed-JSON ×3, `invalid_node_type` ×1), zero new. No trailing-whitespace in the touched files (git-diff-check intent satisfied without invoking git).

### Completion Notes List

- **Task 1 (seam):** New `godot/scripts/ui/flow/run_seed_source.gd` — pure static `RunSeedSource.resolve(configured_seed, entropy) -> Dictionary` (`class_name RunSeedSource extends RefCounted`, mirroring the `RunFlowController` preload-const + global-class pattern). Manual branch (`configured_seed != 0`): seed VERBATIM (no mask/truncation — full int64 preserved), `is_manual_seed = true`, injected entropy ignored. Entropy branch (`configured_seed == 0`): `entropy & 0x7fffffff`, `0 → 1`, `is_manual_seed = false`. Pinned `RESULT_KEYS = ["root_seed", "is_manual_seed"]` (const + asserted). Draws ZERO RNG, reads no autoload/stream, mutates nothing. `.gd.uid` sidecar generated via headless import.
- **Task 2 (hero-select):** `hero_select_presenter._on_confirm_pressed` now routes the seed through the seam and THREADS `is_manual_seed` from it — closing the latent FR28 gap (a configured explicit seed was previously hardcoded `is_manual_seed = false` → wrongly meta-eligible). Deleted `DEFAULT_RUN_SEED = 4242` and `_resolve_seed()`; added the one impure `_new_run_entropy()` line (local `RandomNumberGenerator.new().randomize().randi()` — NOT a named stream, NOT global `randi()`, NOT 1-second `Time`). The 4th `profile` arg on `controller.start(...)` stays absent (deferred Necromancer/Shadeblade concern untouched); class-select gate, reject branch, `set_run_flow` handoff, `route_map` nav all preserved.
- **Task 3 (outpost):** `outpost_presenter._on_descend_pressed` routes the re-descend seed through the same seam. Inline `request` dict keeps its `{root_seed, is_manual_seed, class_id, is_startable}` shape (does NOT call `OutpostViewModel.start_run_request`, so the VM `START_REQUEST_KEYS` pin is untouched) — only the `root_seed` value (now entropy, decimal-string-encoded) and `is_manual_seed` value change. Deleted `DEFAULT_DESCENT_SEED = 4242`; added `_new_run_entropy()`. `is_startable` guard, empty-class legacy start, `clear_run_flow`+`set_run_flow` handoff, `route_map` nav all preserved. In v0 `get_root_seed()` is always 0 → the entropy branch → a fresh, different room every re-descend (F11 fix).
- **Task 4 (test):** New `godot/tests/unit/ui/test_run_seed_source.gd` (beside `test_run_flow_router.gd`) — 6 cases: manual bypass ignores injected entropy; int64 (`INT64_MAX`) preserved verbatim; entropy passthrough stays meta-eligible; normalization non-negative/non-zero across `0`, `-1`, `>31-bit` (all `≤ 0x7fffffff`); two distinct entropies → two distinct seeds; pinned key set. All assert messages use `str(...)` (14.1 test-honesty note).
- **Task 5 (gates):** Only the seam + two presenters touched — no domain/command/event/RNG/save file changed; no new `DomainEvent`, named stream, draw site, autoload, or save key (23-key gate stays 23, `SCHEMA_VERSION == 1`). No test references `_resolve_seed`/`_on_descend_pressed`/a presenter-level seed; the 57 catalog files matching `4242` pass explicit seeds DIRECTLY to the domain (byte-identical, never through the flow entropy source). Every seed-regression/finale/combat-replay fingerprint held (incl. the 14.1-re-pinned combat replay at seed **24680**) — 0 FAIL. 14.4 re-pins nothing.

### File List

- `godot/scripts/ui/flow/run_seed_source.gd` (new — the pure seed-source seam)
- `godot/scripts/ui/flow/run_seed_source.gd.uid` (new — import sidecar, 13.1 discipline)
- `godot/tests/unit/ui/test_run_seed_source.gd` (new — seam unit test)
- `godot/scripts/ui/presenters/hero_select_presenter.gd` (modified — seam-wired "Begin Descent", removed fixed seed, `is_manual_seed` threaded)
- `godot/scripts/ui/presenters/outpost_presenter.gd` (modified — seam-wired "Descend Again", removed fixed seed)

### Review Findings

**Round 1 of 3**

Round 1 primary code review (`gds-code-review`, Opus 4.8, full depth). Diffed three-dot against base `story/14-3-combat-event-log-and-hit-feedback` (merge-base == the 14-3 tip, so two-/three-dot are identical — no already-reviewed 14-1/14-2/14-3 change re-reviewed); spec = this story file. **Verdict: Approve.** Critical 0 / High 0 / Med 0 / Low 2 — 1 open `[Review][Decision]` (a human call, with a recommended default) + 1 `[Review][Defer]`. Scope: the 6 changed `godot/` files — the new pure seam `run_seed_source.gd` (+ its `.gd.uid`), the two flow-layer presenters `hero_select_presenter.gd` / `outpost_presenter.gd`, and the new seam test `test_run_seed_source.gd` (+ its `.gd.uid`); all over `scripts/ui/`.

Verified correct against source:
- **Both live seed sites** ("Begin Descent" + "Descend Again") now route through `RunSeedSource.resolve`; BOTH hardcoded `4242` constants deleted — repo grep: `4242` survives only in comments, and `DEFAULT_RUN_SEED` / `DEFAULT_DESCENT_SEED` / `_resolve_seed` have ZERO remaining references anywhere (incl. tests). Exactly TWO live `.start(` seed callers exist (grep) — no third hardcoded-seed site (no debug/main/smoke caller), so the "fix both, not just hero-select" constraint holds.
- **The seam is pure** (static, draws no RNG, reads no autoload, mutates nothing; entropy INJECTED), pins `RESULT_KEYS = [root_seed, is_manual_seed]` (asserted in both branches, insertion-order stable), and can NEVER emit a seed the domain rejects: `RunStartCommand.validate()` rejects only `root_seed < 0` (`run_start_command.gd:155`) while the entropy branch emits `[1, 2^31-1]` (never < 0, never the 0 sentinel). `_derive_seed` masks the base seed `& 0x7fffffff` (`rng_stream_set.gd:234`), so the seam's 31-bit `SEED_MASK` mirrors the domain and loses no room variety; the 7 named streams are unchanged.
- **No domain/RNG/save/schema change** (diff = 6 files, all under `scripts/ui/flow` + `presenters` + `tests/unit/ui`): the 23-key `RunSnapshot` gate, `SCHEMA_VERSION == 1`, and every named stream are untouched by construction; the FR28 no-meta model is preserved AND a latent gap is CLOSED — hero-select now THREADS `is_manual_seed` from the seam (it was hardcoded `false`, which would have wrongly kept a configured explicit seed meta-eligible). int64 manual seeds round-trip losslessly (hero-select passes the int straight into `controller.start(int(...))`; the outpost's decimal-string `str(int(...))`→`.to_int()` is also lossless, though the outpost manual path is unreachable in v0 with `get_root_seed()` always 0).
- **Suite INDEPENDENTLY re-run on the review head: 201 PASS / 0 FAIL** (baseline 200 + the new seam test). False-PASS guard `SCRIPT ERROR|Parse Error|^FAIL` = 0; exactly the 6 documented stderr negatives reproduced (int64-overflow ×2 / malformed-JSON ×3 / `invalid_node_type` ×1), ZERO new. Every load-bearing determinism fingerprint held — the seed-regression suite, finale (full + regression), generator fairness, all four layout regressions, `rng_stream_set`, `manual_seed_loader`, `run_snapshot`, `run_resume_service`, and the combat replay `test_reference_combat_driver` (seed **24680**) — all PASS. `git diff --check` clean; both new `.gd.uid` sidecars present (13.1 import discipline). 14.4 re-pins nothing.

- [ ] **[Review][Defer]** (Low, AC1/F11 — the live variety fix has no automated guard) — The seam's variety case injects FIXED entropy (`12345` vs `67890`), so it pins the DECISION logic but does NOT exercise the live impure source `_new_run_entropy()` (`RandomNumberGenerator.new().randomize().randi()`). Per AC3 that single OS-entropy line is deliberately un-unit-tested, so the actual F11 fix (two boots → two different rooms; two "Descend Again" taps → two different rooms) rests entirely on Godot 4.6.3's `randomize()` using a high-resolution (µs + PCG-state-mixed) seed — sound for human-paced actions (a human cannot re-descend within one microsecond) and a strict improvement over the story-rejected 1-second `Time.get_unix_time_from_system()`. Non-blocking; consistent with the story's own testability boundary. Owner: the Band-1 on-device observed playtest (boot ≥2× and re-descend ≥2×, confirm different rooms). Logged to `deferred-work.md`.
- [x] **[Review][Decision]** (Low, code-quality — duplicated impure helper) — `_new_run_entropy()` (the 3-line local-RNG entropy read) is duplicated VERBATIM in `hero_select_presenter.gd` (137–140) and `outpost_presenter.gd` (359–362). The impure read MUST stay out of the pure seam (that is exactly what makes the seam unit-testable — AC3), and the story frames it as "a single thin OS-entropy line" per presenter; but two copies can drift if one is later changed. Human call: **keep the per-presenter copy (recommended — 3 trivial lines, matches the story's presenter-boundary framing, and a shared impure helper would add a `flow/` indirection the story did not ask for), OR extract one shared impure helper both presenters call.** No functional impact either way; recommended default: keep as-is.
  - **RESOLVED (human, 2026-07-18): KEEP AS-IS.** Keep the per-presenter 3-line impure `_new_run_entropy()` copy in both `hero_select_presenter.gd` (137–140) and `outpost_presenter.gd` (359–362); do **NOT** extract a shared `flow/` helper. Rationale: the two copies are 3 trivial, verbatim-identical lines; keeping them matches the story's explicit presenter-boundary framing (the impure OS-entropy read is deliberately kept out of the pure, unit-tested seam per AC3), and a shared impure helper would add a `flow/` indirection the story did not ask for. **No production-code change required** — both copies verified byte-identical and left untouched; this dispositions the only open `[Review][Decision]`.
...
| 2026-07-18 | 0.2 | Implemented (gds-dev-story). Added the pure `RunSeedSource` seam (`resolve` → `{root_seed, is_manual_seed}`; manual verbatim/no-truncation + no-meta, entropy 31-bit non-negative non-zero, pinned `RESULT_KEYS`) + its `.gd.uid`; wired BOTH live callers (`hero_select_presenter` "Begin Descent" — now also threads `is_manual_seed`, closing the latent FR28 gap — and `outpost_presenter` "Descend Again"), deleting the two hardcoded seed-4242 sites and adding the single impure `_new_run_entropy()` OS-entropy read per presenter. New `test_run_seed_source.gd` (6 cases). No domain/RNG/save/schema change (23-key gate stays 23, `SCHEMA_VERSION == 1`); every seed-regression/finale/combat-replay (seed 24680) fingerprint byte-identical. Suite 201 PASS / 0 FAIL, guard clean (6 documented negatives). Status → review. | Claude Opus 4.8 (gds-dev-story) |
| 2026-07-18 | 0.3 | Round 1 primary code review (`gds-code-review`, Opus 4.8, full depth; base `story/14-3`). **Verdict: Approve** — Critical 0 / High 0 / Med 0 / Low 2 (1 open `[Review][Decision]` = keep-or-extract the duplicated `_new_run_entropy()`; 1 `[Review][Defer]` = the live F11 variety fix has no automated guard → Band-1 on-device playtest). Verified: both `4242` sites removed (grep-clean, incl. tests), exactly two live `.start(` seed callers, the seam can never emit a domain-rejected seed (`_derive_seed & 0x7fffffff` + `validate` rejects only `< 0`), no domain/RNG/save/schema file touched, the hero-select `is_manual_seed` latent FR28 gap CLOSED. Suite INDEPENDENTLY re-run on the review head: **201 PASS / 0 FAIL**, false-PASS guard clean (6 documented negatives, 0 new), every determinism fingerprint held (incl. combat replay seed 24680), `git diff --check` clean. Findings persisted to `### Review Findings`; the `[Defer]` logged to `deferred-work.md`. No blocking findings — finalize-ready. | Claude Opus 4.8 (gds-code-review) |
| 2026-07-18 | 0.4 | Round 1 review findings dispositioned (gds-dev-story, review-fix mode; NO code change). The single open `[Review][Decision]` (the 3-line impure `_new_run_entropy()` duplicated verbatim in both presenters) resolved by **human as KEEP AS-IS** — per-presenter copy retained in `hero_select_presenter.gd` (137–140) and `outpost_presenter.gd` (359–362); NO shared `flow/` helper extracted; finding ticked + resolution recorded in place. Zero `[Review][Patch]` items. The one `[Review][Defer]` (F11 live-variety fix has no automated guard) left deferred, owned by the Band-1 on-device playtest (already logged in `deferred-work.md`). Verify-only re-run (no production file touched): full headless suite **201 PASS / 0 FAIL**, false-PASS guard `SCRIPT ERROR\|Parse Error\|^FAIL` = 0 (exactly the 6 documented stderr negatives — int64-overflow ×2 / malformed-JSON ×3 / `invalid_node_type` ×1, ZERO new), every determinism fingerprint byte-identical to the review head (incl. combat replay seed 24680); `git diff --check` clean. | Claude Opus 4.8 (gds-dev-story) |
