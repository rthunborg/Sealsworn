...
- [x] 2.8.1 Confirm the Epic 2 save/resume boundary and write failing tests FIRST. (AC: 1-4)
...
- [x] 2.8.7 Run required validation and update story records. (AC: 1-4)
  ...
  - [x] Update this story's Dev Agent Record, Completion Notes, File List, Change Log, and Status. Record the AC3 mid-level feasibility CALL explicitly. Keep `sprint-status.yaml` synchronized with this story's status.
...
- **Implementation (2026-06-14):** Built the domain-side `RunResumeService` (`RefCounted`, save-layer only) that composes the existing restore primitives into the inverse of the Story 2.7 between-level WRITE path: `SaveRepository.read_run_snapshot` → lenient `RunSnapshot.parse` → strict `RunSnapshot.try_tactical_snapshot` → `BoardState.try_from_snapshot` → run-level `RngStreamSet.try_restore`. It returns the restored domain pieces (`run_snapshot`, `tactical_snapshot`, `board`, `rng_streams`) via `ActionResult.ok` on success, and propagates the FIRST validator error (no partial state) on failure. No new save format, no parallel/lenient reader, no new DTO family, no test framework — pure composition of already-tested validators.
- **AC1 (restore + presentation-from-domain):** `test_run_resume_service.gd::_resume_restores_valid_between_level_save` assembles real domain state (`micro_combat_board` + partial fog + 3 RNG draws + turn_state + pending_telegraphs + an event), writes through a real `SaveRepository`, reads back, and asserts `restored_board.to_snapshot() == source.to_snapshot()` (fog flags incl.), turn/telegraph/event-log fidelity, `root_seed`/route round-trip, and that the restored run-level streams reproduce the exact next combat draw. `_resume_payload_exposes_only_domain_state_no_scene_truth` proves the resume payload is JSON-compatible domain objects only — no `Object`/`Node`/`res://`/`.tscn`/audio/presentation references; presentation is rebuilt FROM restored domain state, never from saved scene nodes.
- **AC2 (structured failure / no partial state / recovery path):** five failure cases each through the service — missing file (`save_not_found`), non-JSON garbage (`save_parse_failed`), unsupported schema (`unsupported_save_schema`), corrupt embedded tactical payload (`invalid_tactical_snapshot`), malformed run-level RNG (`invalid_rng_snapshot`). Every failure asserts the stable lower-snake `error_code`, present diagnostic metadata, and NO usable restored `board`/`rng_streams`/`tactical_snapshot`/`run_snapshot` (the `_assert_no_partial_state` helper). `_resume_failed_read_leaves_original_save_intact` proves a failed read leaves the save file byte-for-byte intact (reads never write). The structured `ActionResult.error` IS the domain-layer recovery path a later recovery UI consumes.
- **AC4 (interrupted vs uninterrupted determinism + first-divergence reporting):** `test_resume_flow.gd::_interrupted_path_matches_uninterrupted_path` runs Path A (full fixed sequence of validated `ENTITY_MOVED` events + interleaved RNG draws) and Path B (segment one → between-level save → resume through real JSON write→read → identical segment two), then asserts equality on (1) board snapshot, (2) ordered event log, and (3) RNG state + the exact-next-draw reproduction across all seven streams. Two dedicated helper tests prove the divergence reporters actually NAME the first divergence: `_first_divergent_event_index` returns the first differing event index (or -1) and `_first_divergent_rng_stream` returns the first stream name (or "") — exercised with truncated/mutated logs and an advanced stream. Event logs are compared on a shared JSON-normalized representation so an int-vs-float transport artifact is not mistaken for a real divergence.
- **RNG-authority decision (closes Story 2.7 defer #2):** the run-level `RunSnapshot.rng_streams` is the authoritative between-level RNG on resume; the embedded tactical `rng_streams` reflects in-level state at level exit and EQUALS the run-level one at a between-level boundary by construction. `RunResumeService` restores the run-level streams as the live gameplay streams (documented in a code comment). Equality is now asserted in `test_run_snapshot.gd::_between_level_composes_tactical_snapshot_into_level_state` and `test_run_resume_service.gd::_resume_run_level_streams_equal_embedded_tactical_streams`.
- **Symmetric next-draw determinism (closes Story 2.7 defer #3):** `test_resume_flow.gd::_embedded_tactical_streams_reproduce_next_draw_after_resume` restores the embedded tactical streams and asserts the exact-next-draw reproduction across all seven streams, symmetric with the run-level check.
- **AC3 MID-LEVEL FEASIBILITY CALL — LIMITED (implemented as a demonstrated domain capability; trigger/state-machine wiring deferred to Epics 3-4).** Rationale: the existing Epic 1 `TacticalSnapshot` already serializes EVERYTHING a mid-level snapshot needs — board with per-cell `visible`/`explored` fog memory, entities with positions/HP/occupancy, `turn_state` (holds a mid-turn phase), `pending_telegraphs` (Ash-Seer marks etc.), `rng_streams`, and `event_log`. A mid-level save is therefore mechanically a between-level save taken at an arbitrary turn boundary, and `RunSnapshot.from_between_level(...)` already accepts an arbitrary `turn_state`/`pending_telegraphs`/`event_log`. The genuine gap is NOT the snapshot — it is (a) a level/turn state machine that defines WHEN a mid-level save is safe and (b) a trigger/entry point; both arrive with Epics 3-4. This story therefore demonstrates the capability (no mid-combat trigger wired, correctly out of scope) and proves it with a restore test: `test_run_resume_service.gd::_resume_restores_mid_level_fog_entities_pending_turn_and_rng` takes a save at a mid-turn boundary (partial fog incl. a remembered-not-visible cell, a damaged enemy at partial HP via a validated `DAMAGE_APPLIED` event, a mid-turn `player_action` phase with a non-empty pending telegraph) and asserts faithful restore of all five Sprint-Slice-6 surfaces — fog, entities, pending turn state, RNG stream state, and event log. No `deferred-work.md` mid-level entry is required because the capability is implemented/demonstrated, not deferred; only the trigger/state-machine wiring is out of scope (and is owned by Epics 3-4, not this story).
- **Thin-autoload posture:** added a thin `SaveManager.resume_run(save_path)` delegation to `RunResumeService` (returns the structured `ActionResult` unchanged; no restore/composition logic of its own) — tested for success and a `save_not_found` failure path in `_save_manager_resume_run_delegates_to_service`.
- **GameSession int64-string hardening (Task 2.8.2 latent-fragility note):** `GameSession.restore_rng_snapshot()` previously did `_root_seed = int(snapshot.get("root_seed", _root_seed))`, which would silently truncate a >2^53 `root_seed` now that `RngStreamSet.to_snapshot()` encodes it as a decimal STRING. Changed to read the losslessly-decoded value `try_restore` already returns in its metadata (`result.metadata.get("root_seed")`), preserving the int64-string contract Story 2.7 established. Regression test `_game_session_restore_preserves_full_int64_root_seed` feeds a JSON-round-tripped `9223372036854775000` seed and asserts no truncation. `GameSession` stays thin (no tactical state, no schema policy).
- **Validation:** Godot `4.6.3.stable.official.7d41c59c4`; full headless runner exits `0` with all 40 test files green (38 prior + `test_run_resume_service.gd` + `test_resume_flow.gd`), including the prior `test_run_snapshot.gd` / `test_save_repository.gd` / `test_rng_stream_set.gd` assertions; `git diff --check` reports no whitespace errors. NOTE: the `save_parse_failed` test emits one expected Godot `ERROR: Parse JSON failed` line to stderr — this is `JSON.parse_string`'s own diagnostic when fed the deliberate non-JSON garbage bytes that AC2 requires testing; the test passes and the runner reports success (exit 0).

### File List

- `godot/scripts/save/run_resume_service.gd` (new) — `RunResumeService extends RefCounted`; composes the existing restore primitives into a structured resume; restores run-level RNG as authority; exposes no partial state on failure.
- `godot/scripts/autoloads/save_manager.gd` (modified) — added thin `resume_run(save_path)` delegation to `RunResumeService`; preloads it.
- `godot/scripts/autoloads/game_session.gd` (modified) — `restore_rng_snapshot()` now reads the losslessly-decoded `root_seed` from `try_restore` metadata instead of a raw `int(...)` cast on the int64-string field (prevents >2^53 truncation).
- `godot/tests/unit/save/test_run_resume_service.gd` (new) — AC1 restore round-trip, no-scene-truth, RNG-authority equality, the five AC2 failure cases + no-partial-state + original-file-intact, the AC3 mid-level fog/entities/pending-turn/RNG/event-log restore test, the `SaveManager.resume_run` delegation test, and the `GameSession` int64 root_seed regression.
- `godot/tests/integration/save/test_resume_flow.gd` (new) — AC4 interrupted-vs-uninterrupted determinism through a real JSON write→read, first-divergence reporting (event index + RNG stream) with dedicated reporter-helper tests, and the symmetric embedded-tactical next-draw determinism check.
- `godot/tests/unit/save/test_run_snapshot.gd` (modified) — added the run-level == embedded-tactical `rng_streams` equality assertion (closes a Story 2.7 defer at its named location).
- `_bmad-output/implementation-artifacts/deferred-work.md` (modified) — marked the two Story 2.7 Round-1 RNG-equality / symmetric-next-draw defers RESOLVED in Story 2.8 (the float-tolerance defer remains open as optional hardening).
...
| 2026-06-14 | Implemented `RunResumeService` (domain-side between-level resume) composing the existing restore primitives; added thin `SaveManager.resume_run` delegation; hardened `GameSession.restore_rng_snapshot` int64-string `root_seed` decode. Added AC1/AC2/AC4 + RNG-authority + mid-level (AC3) tests across `test_run_resume_service.gd` and `test_resume_flow.gd`; added the run-level==embedded RNG equality assertion to `test_run_snapshot.gd`. Recorded the AC3 mid-level feasibility call as LIMITED (demonstrated capability; trigger/state-machine wiring deferred to Epics 3-4). Closed two Story 2.7 Round-1 defers. Full headless suite green (exit 0); `git diff --check` clean. Status → review. |

## Review Findings

**Round 1 of 3**

Reviewer: gds-code-review (adversarial: Blind Hunter / Edge Case Hunter / Acceptance Auditor). Date: 2026-06-14. Scope: branch `story/2-8-resume-flow-and-mid-level-save-feasibility` diff vs base `a21c5ee` (6 code files; `_bmad`/`_bmad-output`/cache/non-code excluded). Independent verification: full headless suite re-run green (Godot `4.6.3.stable.official.7d41c59c4`, exit 0, 44 test files incl. the two new ones); `git diff --check` clean on the code diff; `sprint-status.yaml` boundary intact (`epic-1: done`, 2.1-2.7 `done`, 2.8 `review`, 2.9 `backlog`).

**Verdict: Approve.** Critical: 0 / High: 0 / Medium: 0 / Low: 2. The implementation is a faithful composition of the existing restore primitives with no new save format, no parallel/lenient reader, and no weakening of strict validators. All four ACs are covered with real `SaveRepository` JSON write->read transport; AC2's five failure codes (`save_not_found`, `save_parse_failed`, `unsupported_save_schema`, `invalid_tactical_snapshot`, `invalid_rng_snapshot`) match the spec exactly with no-partial-state + diagnostic-metadata + original-file-intact assertions; AC4 divergence reporters are proven to NAME the first divergent event index / RNG stream (not a bare boolean); the AC3 mid-level call is recorded as LIMITED with a restore test over all five Sprint-Slice-6 surfaces; and both folded-in Story 2.7 Round-1 defers (RNG-authority equality, symmetric embedded-tactical next-draw) are genuinely closed. The `GameSession` int64-string `root_seed` hardening is correct: `RngStreamSet.try_restore` returns a native-int `root_seed` in metadata, so the surviving `int(...)` is an identity no-op (verified against `rng_stream_set.gd:160`), and the `9223372036854775000` JSON-round-trip regression test proves no >2^53 truncation. The lone stderr `ERROR: Parse JSON failed` line is `JSON.parse_string`'s own diagnostic on the deliberate garbage bytes the `save_parse_failed` test feeds; the test passes and the runner exits 0 — not a defect. Neither Low finding blocks `done`.

- [x] [Review][Patch] (Low, Blind Hunter) FIXED 2026-06-14 — corrected the comment in `godot/tests/integration/save/test_resume_flow.gd` to state next_sequence_id is 2 after `_new_board`. `godot/tests/integration/save/test_resume_flow.gd:171` comment was factually wrong: it states "the fixture board's next_sequence_id is 1 after `_new_board`", but `_new_board` runs `CreateBoardCommand`, whose applied `BOARD_CREATED` event advances `_next_sequence_id` to 2 (`board_state.gd:368`; consistent with the correct comment in `test_run_resume_service.gd:320`). The test itself is NOT affected — `_apply_segment_one`/`_apply_segment_two` read `board.next_sequence_id()` dynamically rather than hardcoding the value — so this is a documentation-only defect. Risk: a future maintainer copying this reasoning into a test that hardcodes the sequence id would get an `event_sequence_mismatch`. Fix: correct the comment to "is 2 after `_new_board` (the `CreateBoardCommand` `BOARD_CREATED` event consumed sequence 1)".
- [Review][Defer] (Low, Edge Case Hunter) The `save_open_failed` resume failure path (`SaveRepository.read_run_snapshot` returns it when `FileAccess.open` fails on an existing-but-unreadable file) is propagated verbatim by `RunResumeService.resume` but is not exercised by any AC2 test. The spec deliberately scoped AC2 to five deterministic, cross-platform-reproducible cases and did not list `save_open_failed` (a permission/locked-file condition that is awkward to trigger reliably on Windows/CI), and the dev record makes no claim to cover it. Behavior is correct by inspection (the service returns `read_result` unchanged before exposing any state). Not blocking. Defer: if a deterministic unreadable-file harness is later added (e.g. a directory-at-path or a locked handle), add a `save_open_failed` no-partial-state case to `test_run_resume_service.gd` for symmetry with the other read errors. Logged to `deferred-work.md`.
