...
- `godot/tests/unit/ui/test_quit_run_bridge.gd` (+ `.gd.uid`)
...
- `godot/scripts/ui/presenters/gameplay_shell_presenter.gd` — pause overlay + quit-save action.
- `godot/scripts/ui/presenters/hero_select_presenter.gd` — new-run overwrite-confirm + stale-save clear.
- `godot/scripts/ui/presenters/route_map_presenter.gd` — optional between-node route-position autosave (on-screen only).
...
- `godot/tests/unit/ui/test_run_flow_router.gd` — the `boot` + `save_recovery` reachable-target pinning (completed this run).
