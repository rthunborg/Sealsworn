...
**Round 1 of 3** — code review of the `story/2-6-accessibility-and-tactical-readability-baseline` branch diff vs `main`. Reviewer: gds-code-review (Claude Opus 4.8, 1M context), 2026-06-14.

Verdict: **Approve.** Critical 0 / High 0 / Medium 0 / Low 2. The accessibility/readability baseline is implemented as the intended scene-free presentation contract: the cue catalog maps the exact Epic 2 cue-id vocabulary (verified key-by-key against `tactical_movement_preview.gd`, `tactical_attack_preview.gd`, `tactical_inspect_view.gd`, `tactical_attack_commit_flow.gd`) with at least one non-color channel per critical meaning; the net-new `feedback_preview`/`feedback_committed` distinction differs by channel set and survives audio absence; the text-scale clamp/fallback preserves stable reason ids and is proven not to mutate tactical truth via a `TacticalSnapshot.from_domain()` byte-comparison; sanitization is routed through the shared `TacticalPreviewView.safe_*` path (no fourth duplicate sanitizer, honoring the Story 2.5 Round 2 note); and the board-VM `accessibility` slot is the 16th key with its exact sorted-key assertion updated intentionally. Full headless suite green (37 files, exit 0), Godot 4.6.3, `git diff --check` clean. No blocking findings.

Verified during review (no action needed):
- `_preview_active_from_options` correctly requires `kind == "attack"` AND `commit_available`, so a valid movement preview (which also emits `commit_available`) does NOT falsely activate `feedback_preview` — the guard is real and correct.
- `_committed_active_from_options` reads `submitted` + `command_result_summary.succeeded` from the actual `TacticalAttackCommitFlowResult.to_dictionary()` shape; it reads a result and never executes a command. A submitted-but-failed command correctly does not count as committed (matches AC3 "committed/executed").
- `not value is Dictionary/Array` precedence matches the project's established working idiom (`tactical_layout_profile.gd`); the `load(...).new()` factory pattern mirrors `tactical_layout_profile.gd` / `tactical_attack_commit_flow_result.gd`. Both safe.
- AC1/AC2/AC3 each have tests driven from real Epic 2 outputs (`AttackPreviewContractMatrix`, `BoardFixtureFactory`, a live `TacticalAttackCommitFlow`), not hand-rolled stubs.

Findings:

- [Review][Defer] (Low) The `feedback_preview` false-positive guard (`kind == "attack"`) has no regression test. `_feedback_maps_from_active_preview_and_committed_result` only feeds a `commit_flow` in `attack_preview` mode and a committed result; no test passes a valid *movement* preview into `TacticalAccessibilityModel.from_state({"preview": <move_preview>})` to assert `feedback_preview` stays inactive. The guard is correct today (movement previews carry `commit_available` but `kind == "move"`), but a future refactor of `_preview_active_from_options` could silently start activating `feedback_preview` for movement with no test to catch it. Add a focused case: build a valid movement preview via `TacticalMovementPreview.from_query(...)`, pass it as `preview`, and assert `feedback.preview.active == false` and `feedback_preview` is absent from `cue_ids`. Non-blocking — current behavior is verified correct by inspection. (Originating review: Round 1, 2026-06-14.)
- [Review][Decision] (Low) `commit_unavailable` is mapped to channels `["label", "text"]` with severity `blocked`, while `move_preview_invalid` / `attack_preview_invalid` (also `blocked`) use a `shape`/`icon` + `label`. This is internally consistent (every blocked meaning still has a non-color channel, so AC1 holds), but a presenter that renders "commit unavailable" purely as a greyed/disabled button risks leaning on a color/opacity convention; the catalog only guarantees `label`+`text` here, not a `shape`/`icon`. No code change required for AC compliance — flagging for a human/design call on whether the disabled-commit affordance needs an explicit non-color `shape`/`icon` channel added to `commit_unavailable` when the real HUD is built (Story that grows the polished HUD). Decision, not a defect. (Originating review: Round 1, 2026-06-14.)
