...
- [Phase 8 — retrospective] Epic-9 forward contracts: `run_completed.boss_node_id` is boss-placeholder-only (guard on presence — the single hard contract into Epic 9); the live tactical-play loop is now a real Epic-9 dependency (boss finale can't auto-resolve; also the live call site for Epic 7's latent affinity effects). Status-hygiene lag hit 4 consecutive capstones (5-5/6-7/7-6/8-7) — enforce the finalize-checklist status flip, not a soft reminder.
